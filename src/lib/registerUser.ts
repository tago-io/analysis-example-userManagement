...
async function inviteUser(context: TagoContext, account: Account, user_data: UserData, domain_url: string) {
  ...
  emailService.send({
    ...
    message: `Your account for Moka application was created..<br><br>Your Login is: <b>${user_data.email}</b><br>Your password is: <b>${password}</b><br><br> In order to access it, visit our website <a href="${domain_url}">${domain_url}</a>`,
  });
